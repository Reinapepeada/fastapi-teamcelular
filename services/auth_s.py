...
def get_admin_by_identifier(session: Session, identifier: str) -> Admin | None:
    ...
    # 1 query (prioriza match por username si existiera colisión username/email)
    statement = (
        select(Admin)
        .where(or_(Admin.username == identifier, Admin.email == identifier))
        .order_by((Admin.username == identifier).desc())
    )
    return session.exec(statement).first()
...
async def get_current_active_admin(
    ...
    if not current_admin.is_active:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin desactivado")
...
RequireEditorOrHigher = Annotated[
    Admin, Depends(require_role([AdminRole.SUPER_ADMIN, AdminRole.ADMIN, AdminRole.EDITOR]))
]
RequireAdminOrHigher = Annotated[
    Admin, Depends(require_role([AdminRole.SUPER_ADMIN, AdminRole.ADMIN]))
]
