    ...
    if not env_path.exists():
        ...
        if env_example_path.exists():
            ...
            with open(env_path, "w") as dst:
                dst.write(content)
            print(
                f"{Colors.YELLOW}⚠️  Por favor, edita el archivo .env con tus credenciales de PostgreSQL{Colors.END}"
            )
            return False
        else:
...
""")
    ...
    if dev_mode:
        ...
        uvicorn.run(
            "main:app", host=host, port=port, reload=True, reload_dirs=["."], log_level="info"
        )
    else:
        # Modo producción
        import uvicorn

        workers_env = os.getenv("WEB_CONCURRENCY") or os.getenv("UVICORN_WORKERS") or "1"
        try:
