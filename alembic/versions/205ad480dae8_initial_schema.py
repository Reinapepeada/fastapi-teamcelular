...
def enum_exists(conn, enum_name: str) -> bool:
    ...
    result = conn.execute(text("SELECT 1 FROM pg_type WHERE typname = :name"), {"name": enum_name})
...
def upgrade() -> None:
    ...
    create_enum_type(
        conn, "unit", ["KG", "G", "LB", "CM", "M", "INCH", "XS", "S", "L", "XL", "XXL"]
    )
    ...
    if not table_exists(conn, "admin"):
        op.create_table(
            ...
            sa.Column(
                "role",
                sa.Enum("SUPER_ADMIN", "ADMIN", "EDITOR", name="adminrole", create_type=False),
                nullable=False,
            ),
    ...
    if not table_exists(conn, "product"):
        op.create_table(
            ...
            sa.Column(
                "warranty_unit",
                sa.Enum("DAYS", "MONTHS", "YEARS", name="warrantyunit", create_type=False),
                nullable=True,
            ),
            ...
            sa.Column(
                "status",
                sa.Enum(
                    "ACTIVE", "INACTIVE", "DISCONTINUED", name="productstatus", create_type=False
                ),
                nullable=False,
            ),
    ...
    if not table_exists(conn, "productvariant"):
        op.create_table(
            ...
            sa.Column(
                "size_unit",
                sa.Enum(
                    "CLOTHING", "DIMENSIONS", "WEIGHT", "OTHER", name="sizeunit", create_type=False
                ),
                nullable=True,
            ),
            sa.Column(
            ...
            sa.PrimaryKeyConstraint("id"),
            sa.UniqueConstraint(
                "product_id", "color", "size", "size_unit", name="unique_variant_constraint"
            ),
        )
        op.create_index(
            op.f("ix_productvariant_min_stock"), "productvariant", ["min_stock"], unique=False
        )
